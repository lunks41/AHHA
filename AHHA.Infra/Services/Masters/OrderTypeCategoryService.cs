    ...
    public sealed class OrderTypeCategoryService : IOrderTypeCategoryService
    {
        ...
        public async Task<SqlResponce> AddOrderTypeCategoryAsync(string RegId, Int16 CompanyId, M_OrderTypeCategory OrderTypeCategory, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.M_OrderTypeCategory WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.OrderTypeCategory})) AND OrderTypeCategoryCode='{OrderTypeCategory.OrderTypeCategoryCode}' UNION ALL SELECT 2 AS IsExist FROM dbo.M_OrderTypeCategory WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.OrderTypeCategory})) AND OrderTypeCategoryName='{OrderTypeCategory.OrderTypeCategoryName}'");
