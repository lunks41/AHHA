...
namespace AHHA.Infra.Services.Masters
{
    public sealed class UomService : IUomService
    {
        ...
        public async Task<M_Uom> GetUomByIdAsync(Int16 CompanyId, Int16 UomId, Int32 UserId)
        {
            try
            {
                var result = await _repository.GetQuerySingleOrDefaultAsync<M_Uom>($"SELECT UomId,UomCode,UomName,CompanyId,Remarks,IsActive,CreateById,CreateDate,EditById,EditDate FROM dbo.M_Uom WHERE UomId={UomId}");
        ...
        public async Task<SqlResponce> AddUomAsync(Int16 CompanyId, M_Uom Uom, Int32 UserId)
        {
            bool isExist = false;
            ...
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (!isExist)
                    {
                        ...
                        var sqlMissingResponce = await _repository.GetQuerySingleOrDefaultAsync<SqlResponceIds>("SELECT ISNULL((SELECT TOP 1 (UomId + 1) FROM dbo.M_Uom WHERE (UomId + 1) NOT IN (SELECT UomId FROM dbo.M_Uom)),1) AS MissId");
        ...
        public async Task<SqlResponce> UpdateUomAsync(Int16 CompanyId, M_Uom Uom, Int32 UserId)
        {
            ...
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    if (Uom.UomId > 0)
                    {
                        var StrExist = await _repository.GetQueryAsync<SqlResponceIds>($"SELECT 2 AS IsExist FROM dbo.M_Uom WHERE CompanyId IN (SELECT DISTINCT UomId FROM dbo.Fn_Adm_GetShareCompany ({Uom.CompanyId},{(short)Master.Uom},{(short)Modules.Master})) AND UomName='{Uom.UomName} AND UomId <>{Uom.UomId}'");
        ...
        public async Task<SqlResponce> DeleteUomAsync(Int16 CompanyId, M_Uom Uom, Int32 UserId)
        {
            var sqlResponce = new SqlResponce();
            try
            {
