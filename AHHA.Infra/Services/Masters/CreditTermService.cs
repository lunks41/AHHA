    ...
    public sealed class CreditTermService : ICreditTermService
    {
        ...
        public async Task<SqlResponce> AddCreditTermAsync(string RegId, Int16 CompanyId, M_CreditTerm CreditTerm, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.M_CreditTerm WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Master.CreditTerms},{(short)Modules.Master})) AND CreditTermCode='{CreditTerm.CreditTermCode}' UNION ALL SELECT 2 AS IsExist FROM dbo.M_CreditTerm WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Master.CreditTerms},{(short)Modules.Master})) AND CreditTermName='{CreditTerm.CreditTermName}'");
        ...
        public async Task<SqlResponce> UpdateCreditTermAsync(string RegId, Int16 CompanyId, M_CreditTerm CreditTerm, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 2 AS IsExist FROM dbo.M_CreditTerm WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Master.CreditTerms},{(short)Modules.Master})) AND CreditTermName='{CreditTerm.CreditTermName} AND CreditTermId <>{CreditTerm.CreditTermId}'");
