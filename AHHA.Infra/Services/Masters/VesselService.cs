...
namespace AHHA.Infra.Services.Masters
{
    public sealed class VesselService : IVesselService
    {
        ...
        public async Task<M_Vessel> GetVesselByIdAsync(string RegId, Int16 CompanyId, Int32 VesselId, Int32 UserId)
        {
            try
            {
                var result = await _repository.GetQuerySingleOrDefaultAsync<M_Vessel>(RegId, $"SELECT VesselId,VesselCode,VesselName,CompanyId,CallSign,IMOCode,GRT,LicenseNo,VesselType,Flag,Remarks,IsActive,CreateById,CreateDate,EditById,EditDate FROM dbo.M_Vessel WHERE VesselId={VesselId} AND CompanyId IN (SELECT distinct CompanyId FROM Fn_Adm_GetShareCompany({CompanyId},{(short)Modules.Master},{(short)Master.Vessel}))");
        ...
        public async Task<SqlResponce> AddVesselAsync(string RegId, Int16 CompanyId, M_Vessel Vessel, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.M_Vessel WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.Vessel})) AND VesselCode='{Vessel.VesselCode}' UNION ALL SELECT 2 AS IsExist FROM dbo.M_Vessel WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.Vessel})) AND VesselName='{Vessel.VesselName}'");
        ...
        public async Task<SqlResponce> UpdateVesselAsync(string RegId, Int16 CompanyId, M_Vessel Vessel, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 2 AS IsExist FROM dbo.M_Vessel WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.Vessel})) AND VesselName='{Vessel.VesselName} AND VesselId <>{Vessel.VesselId}'");
