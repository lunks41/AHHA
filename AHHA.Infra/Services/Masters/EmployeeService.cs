...
namespace AHHA.Infra.Services.Masters
{
    public sealed class EmployeeService : IEmployeeService
    {
        ...
        public async Task<M_Employee> GetEmployeeByIdAsync(string RegId, Int16 CompanyId, Int32 EmployeeId, Int32 UserId)
        {
            try
            {
                var result = await _repository.GetQuerySingleOrDefaultAsync<M_Employee>(RegId, $"SELECT EmployeeId,CompanyId,EmployeeCode,EmployeeName,EmployeeOtherName,EmployeePhoto,EmployeeSignature,DepartmentId,EmployeeSex,MartialStatus,EmployeeDOB,EmployeeJoinDate,EmployeeLastDate,EmployeeOffEmailAdd,EmployeeOtherEmailAdd,Remarks,IsActive,CreateById,CreateDate,EditById,EditDate FROM dbo.M_Employee WHERE EmployeeId={EmployeeId} AND CompanyId IN (SELECT distinct CompanyId FROM Fn_Adm_GetShareCompany({CompanyId},{(short)Modules.Master},{(short)Master.Employee}))");
