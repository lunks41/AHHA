    ...
    public sealed class GroupCreditLimit_CustomerService : IGroupCreditLimit_CustomerService
    {
        ...
        public async Task<SqlResponce> AddGroupCreditLimit_CustomerAsync(string RegId, Int16 CompanyId, M_GroupCreditLimit_Customer GroupCreditLimit_Customer, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.M_GroupCreditLimit_Customer WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.GroupCreditLimit_Customer})) UNION ALL SELECT 2 AS IsExist FROM dbo.M_GroupCreditLimit_Customer WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.GroupCreditLimit_Customer}))'");
        ...
        public async Task<SqlResponce> UpdateGroupCreditLimit_CustomerAsync(string RegId, Int16 CompanyId, M_GroupCreditLimit_Customer GroupCreditLimit_Customer, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 2 AS IsExist FROM dbo.M_GroupCreditLimit_Customer WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.GroupCreditLimit_Customer}))  AND GroupCreditLimitId <>{GroupCreditLimit_Customer.GroupCreditLimitId}'");
