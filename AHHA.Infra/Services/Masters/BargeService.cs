...
namespace AHHA.Infra.Services.Masters
{
    public sealed class BargeService : IBargeService
    {
        ...
        public async Task<M_Barge> GetBargeByIdAsync(string RegId, Int16 CompanyId, Int16 BargeId, Int32 UserId)
        {
            try
            {
                var result = await _repository.GetQuerySingleOrDefaultAsync<M_Barge>(RegId, $"SELECT BargeId,CompanyId,BargeCode,BargeName,CallSign,IMOCode,GRT,LicenseNo,BargeType,Flag,Remarks,IsActive,CreateById,CreateDate,EditById,EditDate FROM dbo.M_Barge WHERE BargeId={BargeId} AND CompanyId IN (SELECT distinct CompanyId FROM Fn_Adm_GetShareCompany({CompanyId},{(short)Modules.Master},{(short)Master.Barge}))");
        ...
        public async Task<SqlResponce> AddBargeAsync(string RegId, Int16 CompanyId, M_Barge Barge, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.M_Barge WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.Barge})) AND BargeCode='{Barge.BargeId}' UNION ALL SELECT 2 AS IsExist FROM dbo.M_Barge WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.Barge})) AND BargeName='{Barge.BargeName}'");
        ...
        public async Task<SqlResponce> UpdateBargeAsync(string RegId, Int16 CompanyId, M_Barge Barge, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    if (Barge.BargeId > 0)
                    {
                        var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 2 AS IsExist FROM dbo.M_Barge WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.Barge})) AND BargeName='{Barge.BargeName} AND BargeId <>{Barge.BargeId}'");
