...
namespace AHHA.Infra.Services.Masters
{
    public sealed class GstService : IGstService
    {
        ...
        public async Task<M_Gst> GetGstByIdAsync(Int16 CompanyId, Int16 GstId, Int32 UserId)
        {
            try
            {
                var result = await _repository.GetQuerySingleOrDefaultAsync<M_Gst>($"SELECT GstId,GstCode,GstName,CompanyId,Remarks,IsActive,CreateById,CreateDate,EditById,EditDate FROM dbo.M_Gst WHERE GstId={GstId}");
        ...
        public async Task<SqlResponce> AddGstAsync(Int16 CompanyId, M_Gst Gst, Int32 UserId)
        {
            bool isExist = false;
            ...
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (!isExist)
                    {
                        ...
                        var sqlMissingResponce = await _repository.GetQuerySingleOrDefaultAsync<SqlResponceIds>("SELECT ISNULL((SELECT TOP 1 (GstId + 1) FROM dbo.M_Gst WHERE (GstId + 1) NOT IN (SELECT GstId FROM dbo.M_Gst)),1) AS MissId");
        ...
        public async Task<SqlResponce> UpdateGstAsync(Int16 CompanyId, M_Gst Gst, Int32 UserId)
        {
            ...
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    if (Gst.GstId > 0)
                    {
                        var StrExist = await _repository.GetQueryAsync<SqlResponceIds>($"SELECT 2 AS IsExist FROM dbo.M_Gst WHERE CompanyId IN (SELECT DISTINCT GstId FROM dbo.Fn_Adm_GetShareCompany ({Gst.CompanyId},{(short)Master.Gst},{(short)Modules.Master})) AND GstName='{Gst.GstName} AND GstId <>{Gst.GstId}'");
        ...
        public async Task<SqlResponce> DeleteGstAsync(Int16 CompanyId, M_Gst Gst, Int32 UserId)
        {
            var sqlResponce = new SqlResponce();
            try
            {
