...
namespace AHHA.Infra.Services.Masters
{
    public sealed class BankService : IBankService
    {
        ...
        public async Task<M_Bank> GetBankByIdAsync(string RegId, Int16 CompanyId, Int16 BankId, Int32 UserId)
        {
            try
            {
                var result = await _repository.GetQuerySingleOrDefaultAsync<M_Bank>(RegId, $"SELECT BankId,CompanyId,BankCode,BankName,CurrencyId,AccountNo,SwiftCode,Remarks1,Remarks2,GLId,IsActive,CreateById,CreateDate,EditById,EditDate FROM dbo.M_Bank WHERE BankId={BankId} AND CompanyId IN (SELECT distinct CompanyId FROM Fn_Adm_GetShareCompany({CompanyId},{(short)Modules.Master},{(short)Master.Bank}))");
        ...
        public async Task<SqlResponce> UpdateBankAsync(string RegId, Int16 CompanyId, M_Bank Bank, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    if (Bank.BankId > 0)
                    {
                        var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 2 AS IsExist FROM dbo.M_Bank WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.Bank})) AND BankName='{Bank.BankName} AND BankId <>{Bank.BankId}'");
