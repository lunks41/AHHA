...
namespace AHHA.Infra.Services.Masters
{
    public sealed class DepartmentService : IDepartmentService
    {
        ...
        public async Task<M_Department> GetDepartmentByIdAsync(Int16 CompanyId, Int32 DepartmentId, Int32 UserId)
        {
            try
            {
                var result = await _repository.GetQuerySingleOrDefaultAsync<M_Department>($"SELECT DepartmentId,DepartmentCode,DepartmentName,CompanyId,Remarks,IsActive,CreateById,CreateDate,EditById,EditDate FROM dbo.M_Department WHERE DepartmentId={DepartmentId}");
        ...
        public async Task<SqlResponce> AddDepartmentAsync(Int16 CompanyId, M_Department Department, Int32 UserId)
        {
            bool isExist = false;
            ...
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (!isExist)
                    {
                        ...
                        var sqlMissingResponce = await _repository.GetQuerySingleOrDefaultAsync<SqlResponceIds>("SELECT ISNULL((SELECT TOP 1 (DepartmentId + 1) FROM dbo.M_Department WHERE (DepartmentId + 1) NOT IN (SELECT DepartmentId FROM dbo.M_Department)),1) AS MissId");
        ...
        public async Task<SqlResponce> UpdateDepartmentAsync(Int16 CompanyId, M_Department Department, Int32 UserId)
        {
        ...
        public async Task<SqlResponce> DeleteDepartmentAsync(Int16 CompanyId, M_Department Department, Int32 UserId)
        {
            var sqlResponce = new SqlResponce();
            try
            {
