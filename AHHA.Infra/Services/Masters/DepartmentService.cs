    ...
    public sealed class DepartmentService : IDepartmentService
    {
        ...
        public async Task<SqlResponce> AddDepartmentAsync(string RegId, Int16 CompanyId, M_Department Department, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.M_Department WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.Department})) AND DepartmentCode='{Department.DepartmentCode}' UNION ALL SELECT 2 AS IsExist FROM dbo.M_Department WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.Department})) AND DepartmentName='{Department.DepartmentName}'");
        ...
        public async Task<SqlResponce> UpdateDepartmentAsync(string RegId, Int16 CompanyId, M_Department Department, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 2 AS IsExist FROM dbo.M_Department WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.Department})) AND DepartmentName='{Department.DepartmentName} AND DepartmentId <>{Department.DepartmentId}'");
