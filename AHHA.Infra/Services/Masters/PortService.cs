...
namespace AHHA.Infra.Services.Masters
{
    public sealed class PortService : IPortService
    {
        ...
        public async Task<M_Port> GetPortByIdAsync(Int16 CompanyId, Int32 PortId, Int32 UserId)
        {
            try
            {
                var result = await _repository.GetQuerySingleOrDefaultAsync<M_Port>($"SELECT PortId,PortCode,PortName,CompanyId,Remarks,IsActive,CreateById,CreateDate,EditById,EditDate FROM dbo.M_Port WHERE PortId={PortId}");
        ...
        public async Task<SqlResponce> AddPortAsync(Int16 CompanyId, M_Port Port, Int32 UserId)
        {
            bool isExist = false;
            ...
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (!isExist)
                    {
                        ...
                        var sqlMissingResponce = await _repository.GetQuerySingleOrDefaultAsync<SqlResponceIds>("SELECT ISNULL((SELECT TOP 1 (PortId + 1) FROM dbo.M_Port WHERE (PortId + 1) NOT IN (SELECT PortId FROM dbo.M_Port)),1) AS MissId");
        ...
        public async Task<SqlResponce> UpdatePortAsync(Int16 CompanyId, M_Port Port, Int32 UserId)
        {
            ...
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    if (Port.PortId > 0)
                    {
                        var StrExist = await _repository.GetQueryAsync<SqlResponceIds>($"SELECT 2 AS IsExist FROM dbo.M_Port WHERE CompanyId IN (SELECT DISTINCT PortId FROM dbo.Fn_Adm_GetShareCompany ({Port.CompanyId},{(short)Master.Port},{(short)Modules.Master})) AND PortName='{Port.PortName} AND PortId <>{Port.PortId}'");
        ...
        public async Task<SqlResponce> DeletePortAsync(Int16 CompanyId, M_Port Port, Int32 UserId)
        {
            var sqlResponce = new SqlResponce();
            try
            {
