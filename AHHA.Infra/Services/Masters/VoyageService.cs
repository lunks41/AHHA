    ...
    public sealed class VoyageService : IVoyageService
    {
        ...
        public async Task<SqlResponce> AddVoyageAsync(string RegId, Int16 CompanyId, M_Voyage Voyage, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.M_Voyage WHERE CompanyId IN (SELECT DISTINCT VoyageId FROM dbo.Fn_Adm_GetShareCompany ({Voyage.CompanyId},{(short)Modules.Master},{(short)Master.Voyage})) AND VoyageNo='{Voyage.VoyageNo}' UNION ALL SELECT 2 AS IsExist FROM dbo.M_Voyage WHERE CompanyId IN (SELECT DISTINCT VoyageId FROM dbo.Fn_Adm_GetShareCompany ({Voyage.CompanyId},{(short)Modules.Master},{(short)Master.Voyage})) ");
        ...
        public async Task<SqlResponce> UpdateVoyageAsync(string RegId, Int16 CompanyId, M_Voyage Voyage, Int32 UserId)
        {
            ...
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 2 AS IsExist FROM dbo.M_Voyage WHERE CompanyId IN (SELECT DISTINCT VoyageId FROM dbo.Fn_Adm_GetShareCompany ({Voyage.CompanyId},{(short)Modules.Master},{(short)Master.Voyage})) AND VoyageNo='{Voyage.VoyageNo} AND VoyageId <>{Voyage.VoyageId}'");
