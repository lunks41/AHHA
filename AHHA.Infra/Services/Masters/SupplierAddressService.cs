...
namespace AHHA.Infra.Services.Masters
{
    public sealed class SupplierAddressService : ISupplierAddressService
    {
        ...
        public async Task<M_SupplierAddress> GetSupplierAddressByIdAsync(Int16 CompanyId, Int32 AddressId, Int32 UserId)
        {
            try
            {
                var result = await _repository.GetQuerySingleOrDefaultAsync<M_SupplierAddress>($"SELECT AddressId,SupplierAddressCode,SupplierAddressName,CompanyId,Remarks,IsActive,CreateById,CreateDate,EditById,EditDate FROM dbo.M_SupplierAddress WHERE AddressId={AddressId}");
        ...
        public async Task<SqlResponce> AddSupplierAddressAsync(Int16 CompanyId, M_SupplierAddress SupplierAddress, Int32 UserId)
        {
            bool isExist = false;
            ...
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (StrExist.Count() > 0)
                    {
                        ...
                        {
                            isExist = true;
                            return new SqlResponce { Id = -1, Message = "SupplierAddress Code Exist" };
                        }
                        else if (StrExist.ToList()[1].IsExist == 2)
                        {
                            isExist = true;
                            return new SqlResponce { Id = -2, Message = "SupplierAddress Name Exist" };
                        }
                    }
                    else
                    {
                    ...
                    if (!isExist)
                    {
                        ...
                        var sqlMissingResponce = await _repository.GetQuerySingleOrDefaultAsync<SqlResponceIds>("SELECT ISNULL((SELECT TOP 1 (AddressId + 1) FROM dbo.M_SupplierAddress WHERE (AddressId + 1) NOT IN (SELECT AddressId FROM dbo.M_SupplierAddress)),1) AS MissId");
        ...
        public async Task<SqlResponce> UpdateSupplierAddressAsync(Int16 CompanyId, M_SupplierAddress SupplierAddress, Int32 UserId)
        {
        ...
        public async Task<SqlResponce> DeleteSupplierAddressAsync(Int16 CompanyId, M_SupplierAddress SupplierAddress, Int32 UserId)
        {
            var sqlResponce = new SqlResponce();
            try
            {
