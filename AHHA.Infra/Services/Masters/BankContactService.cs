...
namespace AHHA.Infra.Services.Masters
{
    public sealed class BankContactService : IBankContactService
    {
        ...
        public async Task<SqlResponce> SaveBankContactAsync(string RegId, Int16 CompanyId, M_BankContact m_BankContact, Int16 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                ...
                try
                {
                    ...
                    if (IsEdit)
                    {
                        var DataExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.M_BankContact where BankId = {m_BankContact.BankId} And ContactName = '{m_BankContact.ContactName}' And ContactId<>{m_BankContact.ContactId}");
                    ...
                    {
                        var sqlMissingResponce = await _repository.GetQuerySingleOrDefaultAsync<SqlResponceIds>(RegId, $"SELECT ISNULL((SELECT TOP 1(ContactId + 1) FROM dbo.M_BankContact WHERE BankId = {m_BankContact.BankId} AND (ContactId + 1) NOT IN(SELECT ContactId FROM dbo.M_BankContact where BankId= {m_BankContact.BankId})),1) AS NextId");

                        if (sqlMissingResponce != null && sqlMissingResponce.NextId > 0)
                        {
                            m_BankContact.ContactId = Convert.ToInt16(sqlMissingResponce.NextId);
        ...
        public async Task<SqlResponce> DeleteBankContactAsync(string RegId, Int16 CompanyId, Int16 BankId, Int16 ContactId, Int16 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
