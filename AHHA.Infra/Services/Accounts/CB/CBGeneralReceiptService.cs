        ...
        public async Task<SqlResponce> SaveCBGeneralReceiptAsync(string RegId, Int16 CompanyId, CBGenReceiptHd CBGenReceiptHd, List<CBGenReceiptDt> CBGenReceiptDt, Int16 UserId)
        {
            ...
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    {
                        await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"exec FIN_AR_CreateHistoryRec {CompanyId},{UserId},{CBGenReceiptHd.ReceiptId},{(short)E_AR.Invoice}");
                    }
                    ...
                    if (SaveHeader > 0)
                    {
                        ...
                        if (SaveDetails > 0)
                        {
                            ...
                            await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"exec FIN_AR_CreateStatement {CompanyId},{UserId},{CBGenReceiptHd.ReceiptId},{(short)E_AR.Invoice}");
                ...
                using (TransactionScope TScope = new TransactionScope())
                {
                    ...
                    if (ReceiptId > 0)
                    {
                        ...
                        if (CBGeneralReceiptToRemove > 0)
                        {
                            ...
                            await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"exec FIN_AR_DeleteStatement {CompanyId},{UserId},{ReceiptId},{(short)E_AR.Invoice}");
