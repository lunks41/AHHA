...
namespace AHHA.Infra.Services.Accounts.CB
{
    public sealed class CBGenReceiptService : ICBGenReceiptService
    {
        ...
        public async Task<CBGenReceiptHdViewModel> GetCBGenReceiptByIdNoAsync(string RegId, Int16 CompanyId, Int64 InvoiceId, string InvoiceNo, Int16 UserId)
        {
            CBGenReceiptHdViewModel CBGenReceiptViewModel = new CBGenReceiptHdViewModel();
            try
            {
                CBGenReceiptViewModel = await _repository.GetQuerySingleOrDefaultAsync<CBGenReceiptHdViewModel>(RegId, $"SELECT Invhd.CompanyId,Invhd.ReceiptId,Invhd.ReceiptNo,Invhd.ReferenceNo,Invhd.TrnDate,Invhd.AccountDate,Invhd.CurrencyId,M_Cur.CurrencyCode,M_Cur.CurrencyCode,Invhd.ExhRate,Invhd.CtyExhRate,Invhd.BankId,M_Ban.BankCode,M_Ban.BankName,Invhd.PaymentTypeId,M_Pay.PaymentTypeCode,M_Pay.PaymentTypeName,Invhd.ChequeNo,Invhd.ChequeDate,Invhd.BankChgAmt,Invhd.BankChgLocalAmt,Invhd.TotAmt,Invhd.TotLocalAmt,Invhd.TotCtyAmt,Invhd.GstClaimDate,Invhd.GstAmt,Invhd.GstLocalAmt,Invhd.GstCtyAmt,Invhd.TotAmtAftGst,Invhd.TotLocalAmtAftGst,Invhd.TotCtyAmtAftGst,Invhd.Remarks,Invhd.PayeeTo,Invhd.ModuleFrom,Invhd.CreateById,Invhd.CreateDate,Invhd.EditById,Invhd.EditDate,Invhd.IsCancel,Invhd.CancelById,Invhd.CancelDate,Invhd.CancelRemarks,Usr.UserName AS CreateBy,Usr1.UserName AS EditBy,Usr2.UserName AS CancelBy,Invhd.EditVersion FROM dbo.CBGenReceiptHd Invhd INNER JOIN dbo.M_Currency M_Cur ON M_Cur.CurrencyId = Invhd.CurrencyId LEFT JOIN dbo.M_PaymentType M_Pay ON M_Pay.PaymentTypeId = Invhd.PaymentTypeId INNER JOIN dbo.M_Bank M_Ban ON M_Ban.BankId = Invhd.BankId LEFT JOIN dbo.AdmUser Usr ON Usr.UserId = Invhd.CreateById LEFT JOIN dbo.AdmUser Usr1 ON Usr1.UserId = Invhd.EditById LEFT JOIN dbo.AdmUser Usr2 ON Usr2.UserId = Invhd.CancelById WHERE (Invhd.ReceiptId={InvoiceId} OR {InvoiceId}=0) AND (Invhd.ReceiptNo='{InvoiceNo}' OR '{InvoiceNo}'='{string.Empty}')");
        ...
        public async Task<SqlResponse> SaveCBGenReceiptAsync(string RegId, Int16 CompanyId, CBGenReceiptHd CBGenReceiptHd, List<CBGenReceiptDt> CBGenReceiptDt, Int16 UserId)
        {
            ...
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    if (IsEdit)
                    {
                        var DataExist = await _repository.GetQueryAsync<SqlResponseIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.CBGenReceiptHd WHERE IsCancel=0 And CompanyId={CompanyId} And InvoiceId={CBGenReceiptHd.ReceiptId}");
                    ...
                    if (SaveHeader > 0)
                    {
                        ...
                        if (SaveDetails > 0)
                        {
                            ...
                            //await _repository.GetQueryAsync<SqlResponseIds>(RegId, $"exec FIN_CB_CreateStatement {CompanyId},{UserId},{CBGenReceiptHd.ReceiptId},{(short)E_CB.CBReceipt}");
                            ...
                            if (auditLogSave > 0)
                            {
                                //Update Edit Version
                                if (IsEdit)
                                    await _repository.UpsertExecuteScalarAsync(RegId, $"update CBGenReceiptHd set EditVersion=EditVersion+1 where InvoiceId={CBGenReceiptHd.ReceiptId}; Update CBGenReceiptDt set EditVersion=(SELECT TOP 1 EditVersion FROM dbo.CBGenReceiptHd where InvoiceId={CBGenReceiptHd.ReceiptId}) where InvoiceId={CBGenReceiptHd.ReceiptId}");
        ...
        public async Task<SqlResponse> DeleteCBGenReceiptAsync(string RegId, Int16 CompanyId, Int64 InvoiceId, string CanacelRemarks, Int16 UserId)
        {
            ...
            try
            {
                ...
                {
                    //Get Invoice Number
                    InvoiceNo = await _repository.GetQuerySingleOrDefaultAsync<string>(RegId, $"SELECT InvoiceNo FROM dbo.CBGenReceiptHd WHERE InvoiceId={InvoiceId}");

                    if (InvoiceId > 0)
                    {
                        //Update IsCancle=1,Cancelby=userid,Canceldate=now,CancelRemarks=CancelRemarks
                        var CBGenReceiptToRemove = _context.CBGenReceiptHd.Where(b => b.ReceiptId == InvoiceId).ExecuteUpdate(setPropertyCalls: setters => setters.SetProperty(b => b.IsCancel, true).SetProperty(b => b.CancelById, UserId).SetProperty(b => b.CancelDate, DateTime.Now).SetProperty(b => b.CancelRemarks, CanacelRemarks));

                        if (CBGenReceiptToRemove > 0)
                        {
                            //Cancel the Ar Transactions.
                            await _repository.GetQueryAsync<SqlResponseIds>(RegId, $"exec FIN_AP_DeleteStatement {CompanyId},{UserId},{InvoiceId},{(short)E_CB.CBReceipt}");
