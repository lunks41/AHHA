...
namespace AHHA.API.Controllers.Setting
{
    ...
    : base(memoryCache, mapper, baseServices)
        ...
        [Authorize]
        public async Task<ActionResult<decimal>> GetGstPercentage(Int16 GstId, DateOnly TrnsDate, [FromHeader] HeaderViewModel headerViewModel)
        {
            try
            {
                if (ValidateHeaders(headerViewModel.RegId, headerViewModel.CompanyId, headerViewModel.UserId))
        ...
        [Authorize]
        public async Task<ActionResult<decimal>> GetCreditTermDay(Int16 CreditTermId, DateOnly TrnsDate, [FromHeader] HeaderViewModel headerViewModel)
        {
            try
            {
                if (ValidateHeaders(headerViewModel.RegId, headerViewModel.CompanyId, headerViewModel.UserId))
