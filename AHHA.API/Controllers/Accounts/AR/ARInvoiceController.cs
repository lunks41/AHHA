...
namespace AHHA.API.Controllers.Accounts.AR
{
    ...
    : base(memoryCache, mapper, baseServices)
        ...
        [HttpGet, Route("GetHistoryARInvoiceDetailsbyId/{InvoiceId}/{EditVersion}")]
        [Authorize]
        public async Task<ActionResult<ARInvoiceViewModel>> GetHistoryARInvoiceDetailsbyId(string InvoiceId, byte EditVersion, [FromHeader] HeaderViewModel headerViewModel)
        {
            try
            {
                if (InvoiceId == null || InvoiceId == "" || InvoiceId == "0")
                ...
                var arInvoiceViewModel = await _ARInvoiceService.GetHistoryARInvoiceDetailsbyId(headerViewModel.RegId, headerViewModel.CompanyId, Convert.ToInt64(string.IsNullOrEmpty(InvoiceId) ? 0 : InvoiceId?.Trim()), EditVersion, string.Empty, headerViewModel.UserId);
