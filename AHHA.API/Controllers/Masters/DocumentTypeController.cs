                ...
                if (ValidateHeaders(headerViewModel.RegId, headerViewModel.CompanyId, headerViewModel.UserId))
                {
                    ...
                    if (userGroupRight != null)
                    {
                        if (userGroupRight.IsCreate)
                        {
                            ...
                            var createdDocumentType = await _DocumentTypeService.SaveDocumentTypeAsync(headerViewModel.RegId, headerViewModel.CompanyId, DocumentTypeEntity, headerViewModel.UserId);
                            return StatusCode(StatusCodes.Status202Accepted, createdDocumentType);
                        }
                        else
                        {
                            return NotFound(GenrateMessage.authenticationfailed);
                ...
                if (ValidateHeaders(headerViewModel.RegId, headerViewModel.CompanyId, headerViewModel.UserId))
                {
                    ...
                    if (userGroupRight != null)
                    {
                        if (userGroupRight.IsDelete)
                        {
                            ...
                            var sqlResponce = await _DocumentTypeService.DeleteDocumentTypeAsync(headerViewModel.RegId, headerViewModel.CompanyId, DocumentTypeToDelete, headerViewModel.UserId);
