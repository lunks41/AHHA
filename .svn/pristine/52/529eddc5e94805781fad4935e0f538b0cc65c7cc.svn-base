    ...
    public sealed class CurrencyService : ICurrencyService
    {
        ...
        public async Task<SqlResponce> AddCurrencyAsync(string RegId, Int16 CompanyId, M_Currency Currency, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.M_Currency WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.Currency})) AND CurrencyCode='{Currency.CurrencyCode}' UNION ALL SELECT 2 AS IsExist FROM dbo.M_Currency WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.Currency})) AND CurrencyName='{Currency.CurrencyName}'");
        ...
        public async Task<SqlResponce> UpdateCurrencyAsync(string RegId, Int16 CompanyId, M_Currency Currency, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 2 AS IsExist FROM dbo.M_Currency WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.Currency})) AND CurrencyName='{Currency.CurrencyName} AND CurrencyId <>{Currency.CurrencyId}'");
