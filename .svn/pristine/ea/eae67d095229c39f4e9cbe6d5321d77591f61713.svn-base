...
namespace AHHA.Infra.Services.Accounts.AR
{
    public sealed class ARRefundService : IARRefundService
    {
        ...
        public ARRefundService(IRepository<ArRefundHd> repository, ApplicationDbContext context, IAccountService accountService)
        {
        ...
        public async Task<ARRefundViewModel> GetARRefundByIdAsync(string RegId, Int16 CompanyId, Int64 RefundId, string RefundNo, Int16 UserId)
        {
            ARRefundViewModel ARRefundViewModel = new ARRefundViewModel();
            try
            {
                ARRefundViewModel = await _repository.GetQuerySingleOrDefaultAsync<ARRefundViewModel>(RegId, $"SELECT Rpthd.CompanyId,Rpthd.RefundId,Rpthd.RefundNo,Rpthd.ReferenceNo,Rpthd.TrnDate,Rpthd.AccountDate,Rpthd.BankId,M_Ban.BankCode,M_Ban.BankName,Rpthd.PaymentTypeId,M_Pay.PaymentTypeCode,M_Pay.PaymentTypeName,Rpthd.ChequeNo,Rpthd.ChequeDate,Rpthd.CustomerId,M_Cus.CustomerCode,M_Cus.CustomerName,Rpthd.CurrencyId,M_Cur.CurrencyCode,M_Cur.CurrencyCode,Rpthd.ExhRate,Rpthd.TotAmt,Rpthd.TotLocalAmt,Rpthd.UnAllocTotAmt,Rpthd.UnAllocTotLocalAmt,Rpthd.RecCurrencyId,R_Cur.CurrencyCode RecCurrencyCode,R_Cur.CurrencyName RecCurrencyName,Rpthd.RecExhRate,Rpthd.RecTotAmt,Rpthd.RecTotLocalAmt,Rpthd.ExhGainLoss,Rpthd.AllocTotAmt,Rpthd.AllocTotLocalAmt,Rpthd.BankChargeGLId,M_chr.GLCode BankChargeGLCode,M_chr.GLName BankChargeGLName,Rpthd.BankChargesAmt,Rpthd.BankChargesLocalAmt,Rpthd.ModuleFrom,Rpthd.CreateById,Rpthd.CreateDate,Rpthd.EditById,Rpthd.EditDate,Rpthd.CancelById,Rpthd.CancelDate,Rpthd.CancelRemarks,Usr.UserName AS CreateBy,Usr1.UserName AS EditBy,Usr2.UserName AS CancelBy,Rpthd.EditVersion FROM dbo.ArRefundHd Rpthd INNER JOIN dbo.M_Customer M_Cus ON M_Cus.CustomerId = Rpthd.CustomerId INNER JOIN dbo.M_Currency M_Cur ON M_Cur.CurrencyId = Rpthd.CurrencyId INNER JOIN dbo.M_Currency R_Cur ON R_Cur.CurrencyId = Rpthd.RecCurrencyId INNER JOIN dbo.M_PaymentType M_Pay ON M_Pay.PaymentTypeId = Rpthd.PaymentTypeId INNER JOIN dbo.M_Bank M_Ban ON M_Ban.BankId = Rpthd.BankId LEFT JOIN dbo.AdmUser Usr ON Usr.UserId = Rpthd.CreateById LEFT JOIN dbo.AdmUser Usr1 ON Usr1.UserId = Rpthd.EditById LEFT JOIN dbo.AdmUser Usr2 ON Usr2.UserId = Rpthd.CancelById LEFT JOIN dbo.M_ChartOfAccount M_chr ON M_chr.GLId = Rpthd.BankChargeGLId WHERE (Rpthd.RefundId={RefundId} OR {RefundId}=0) AND (Rpthd.RefundNo='{RefundNo}' OR '{RefundNo}'='{string.Empty}')");

                if (ARRefundViewModel == null)
                    return ARRefundViewModel;

                var result = await _repository.GetQueryAsync<ARRefundDtViewModel>(RegId, $"SELECT Rptdt.CompanyId,Rptdt.RefundId,Rptdt.RefundNo,Rptdt.ItemNo,Rptdt.TransactionId,Rptdt.DocumentId,Rptdt.DocumentNo,Rptdt.ReferenceNo,Rptdt.DocCurrencyId,Rptdt.DocExhRate,Rptdt.DocAccountDate,Rptdt.DocDueDate,Rptdt.DocTotAmt,Rptdt.DocTotLocalAmt,Rptdt.DocBalAmt,Rptdt.DocBalLocalAmt,Rptdt.AllocAmt,Rptdt.AllocLocalAmt,Rptdt.DocAllocAmt,Rptdt.DocAllocLocalAmt,Rptdt.CentDiff,Rptdt.ExhGainLoss FROM dbo.ArRefundDt Rptdt  WHERE Rptdt.RefundId={ARRefundViewModel.RefundId}");
        ...
        public async Task<SqlResponse> SaveARRefundAsync(string RegId, Int16 CompanyId, ArRefundHd arRefundHd, List<ArRefundDt> arRefundDts, Int16 UserId)
        {
            bool IsEdit = false;
            ...
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    if (!IsPeriodClosed)
                    {
                        ...
                        if (IsEdit)
                        {
                            var DataExist = await _repository.GetQueryAsync<SqlResponseIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.ArRefundHd WHERE IsCancel=0 And CompanyId={CompanyId} And RefundId={arRefundHd.RefundId}");
                        ...
                        if (!IsEdit)
                        {
                            var documentIdNo = await _repository.GetQueryAsync<SqlResponseIds>(RegId, $"exec S_GENERATE_NUMBER_NOANDID {CompanyId},{(short)E_Modules.AR},{(short)E_AR.Refund},'{accountDate}'");
                        ...
                        {
                            //Insert the previous ARRefund record to ARRefund history table as well as editversion also.
                        ...
                        if (SaveHeader > 0)
                        {
                            ...
                            if (SaveDetails > 0)
                            {
                                ...
                                if (auditLogSave > 0)
                                {
                                    //Update Edit Version
                                    if (IsEdit)
                                        await _repository.UpsertExecuteScalarAsync(RegId, $"update ArRefundHd set EditVersion=EditVersion+1 where RefundId={arRefundHd.RefundId}; Update ArRefundDt set EditVersion=(SELECT TOP 1 EditVersion FROM dbo.ArRefundHd where RefundId={arRefundHd.RefundId}) where RefundId={arRefundHd.RefundId}");
        ...
        public async Task<SqlResponse> DeleteARRefundAsync(string RegId, Int16 CompanyId, Int64 RefundId, string RefundNo, string CanacelRemarks, Int16 UserId)
        {
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
        ...
        public async Task<IEnumerable<ARRefundViewModel>> GetHistoryARRefundByIdAsync(string RegId, Int16 CompanyId, Int64 RefundId, string RefundNo, Int16 UserId)
        {
            try
            {
