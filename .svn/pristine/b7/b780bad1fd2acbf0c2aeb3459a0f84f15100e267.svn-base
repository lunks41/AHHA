...
namespace AHHA.Infra.Services.Setting
{
    public sealed class DecSettingsServices : IDecSettingsService
    {
        ...
        public async Task<S_DecSettings> GetDecSettingAsync(string RegId, Int16 CompanyId, Int16 UserId)
        {
            try
            {
                var result = await _repository.GetQuerySingleOrDefaultAsync<S_DecSettings>(RegId, $"SELECT CompanyId,AmtDec,LocAmtDec,PriceDec,QtyDec,ExhRateDec,DateFormat,CreateById,CreateDate,EditById,EditDate FROM dbo.S_DecSettings WHERE CompanyId IN (SELECT distinct CompanyId FROM Fn_Adm_GetShareCompany({CompanyId},{(short)E_Modules.Admin},{(short)E_Admin.DecSetting}))");
        ...
        public async Task<SqlResponce> UpsertDecSettingAsync(string RegId, Int16 CompanyId, S_DecSettings s_DecSettings, Int16 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.S_DecSettings WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)E_Modules.Admin},{(short)E_Admin.DecSetting}))");
