    ...
    public sealed class NumberFormatServices : INumberFormatServices
    {
        ...
        public async Task<SqlResponce> AddNumberFormatAsync(string RegId, Int16 CompanyId, S_NumberFormat s_NumberFormat, Int16 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.S_NumberFormat WHERE ModuleId={s_NumberFormat.ModuleId} AND TransactionId={s_NumberFormat.TransactionId} AND PrefixSeq='{s_NumberFormat.PrefixSeq}' AND CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)E_Modules.Admin},{(short)E_Admin.NumberFormat}))");
                    ...
                    var sqlMissingResponce = await _repository.GetQuerySingleOrDefaultAsync<SqlResponceIds>(RegId, "SELECT ISNULL((SELECT TOP 1 (NumberId + 1) FROM dbo.S_NumberFormat WHERE (NumberId + 1) NOT IN (SELECT NumberId FROM dbo.S_NumberFormat)),1) AS MissId");

                    if (sqlMissingResponce != null && sqlMissingResponce.MissId > 0)
                    {
                        #region Saving NumberFormat

                        s_NumberFormat.NumberId = Convert.ToInt32(sqlMissingResponce.MissId);
        ...
        public async Task<SqlResponce> UpdateNumberFormatAsync(string RegId, Int16 CompanyId, S_NumberFormat s_NumberFormat, Int16 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.S_NumberFormat WHERE ModuleId={s_NumberFormat.ModuleId} AND TransactionId={s_NumberFormat.TransactionId} AND PrefixSeq='{s_NumberFormat.PrefixSeq}' AND NumberId='{s_NumberFormat.NumberId}' AND CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)E_Modules.Admin},{(short)E_Admin.NumberFormat}))");
