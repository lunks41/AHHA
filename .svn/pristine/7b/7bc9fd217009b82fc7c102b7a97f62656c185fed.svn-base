    ...
    public sealed class ChartOfAccountService : IChartOfAccountService
    {
        ...
        public async Task<SqlResponse> SaveChartOfAccountAsync(string RegId, Int16 CompanyId, M_ChartOfAccount m_ChartOfAccount, Int16 UserId)
        {
            using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                ...
                try
                {
                    var codeExist = await _repository.GetQuerySingleOrDefaultAsync<SqlResponseIds>(RegId, $"SELECT TOP (1) 1 AS IsExist FROM dbo.M_ChartOfAccount WHERE GLId<>{m_ChartOfAccount.GLId} AND GLCode={m_ChartOfAccount.GLCode} AND CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)E_Modules.Master},{(short)E_Master.ChartOfAccount}))");

                    if (codeExist.IsExist > 0)
                        return new SqlResponse { Result = -1, Message = "Gl Code exists" };

                    var nameExist = await _repository.GetQuerySingleOrDefaultAsync<SqlResponseIds>(RegId, $"SELECT TOP (1) 1 AS IsExist FROM dbo.M_ChartOfAccount WHERE GLId<>{m_ChartOfAccount.GLId} AND GLName={m_ChartOfAccount.GLName} AND CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)E_Modules.Master},{(short)E_Master.ChartOfAccount}))");

                    if (nameExist.IsExist > 0)
                    ...
                    if (IsEdit)
                    {
                        var dataExist = await _repository.GetQuerySingleOrDefaultAsync<SqlResponseIds>(RegId, $"SELECT TOP (1) 1 AS IsExist FROM dbo.M_ChartOfAccount WHERE GLId<>0 AND GLId={m_ChartOfAccount.GLId} AND CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)E_Modules.Master},{(short)E_Master.ChartOfAccount}))");

                        if (dataExist.IsExist > 0)
                        {
                    ...
                    {
                        var sqlMissingResponse = await _repository.GetQuerySingleOrDefaultAsync<SqlResponseIds>(RegId, "SELECT ISNULL((SELECT TOP 1 (GLId + 1) FROM dbo.M_ChartOfAccount WHERE (GLId + 1) NOT IN (SELECT GLId FROM dbo.M_ChartOfAccount)),1) AS NextId");
                    ...
                    if (saveChangeRecord > 0)
                    {
                        ...
                        if (auditLogSave > 0)
                        {
                            var dataExist = await _repository.GetQuerySingleOrDefaultAsync<M_ChartOfAccount>(RegId, $"SELECT * FROM dbo.M_ChartOfAccount WHERE GLId<>0 AND GLId={m_ChartOfAccount.GLId} AND CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)E_Modules.Master},{(short)E_Master.ChartOfAccount}))");
