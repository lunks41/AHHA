    ...
    public sealed class GroupCreditLimitService : IGroupCreditLimitService
    {
        ...
        public async Task<SqlResponce> AddGroupCreditLimitAsync(string RegId, Int16 CompanyId, M_GroupCreditLimit GroupCreditLimit, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.M_GroupCreditLimit WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.GroupCreditLimit})) UNION ALL SELECT 2 AS IsExist FROM dbo.M_GroupCreditLimit WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.GroupCreditLimit}))'");
        ...
        public async Task<SqlResponce> UpdateGroupCreditLimitAsync(string RegId, Int16 CompanyId, M_GroupCreditLimit GroupCreditLimit, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 2 AS IsExist FROM dbo.M_GroupCreditLimit WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.GroupCreditLimit}))  AND GroupCreditLimitId <>{GroupCreditLimit.GroupCreditLimitId}'");
