...
namespace AHHA.Infra.Services.Admin
{
    public sealed class UserLogService : IUserLogService
    {
        ...
        public UserLogService(IRepository<AdmUserLog> repository, ApplicationDbContext context)
        {
        ...
        public async Task<UserLogViewModelCount> GetUserLogListAsync(string RegId, Int16 CompanyId, short pageSize, short pageNumber, string searchString, Int16 UserId)
        {
            ...
            try
            {
                var totalcount = await _repository.GetQuerySingleOrDefaultAsync<SqlResponceIds>(RegId, $"SELECT COUNT(*) AS CountId FROM dbo.AdmUserLog A_UsrLog INNER JOIN dbo.AdmUser A_Usr ON A_Usr.UserId = A_UsrLog.UserId WHERE (A_Usr.UserCode LIKE '%{searchString}%' OR A_Usr.UserName LIKE '%{searchString}%' OR A_UsrLog.Remarks LIKE '%{searchString}%')");
        ...
        public async Task<SqlResponce> AddUserLog(string RegId, Int16 CompanyId, AdmUserLog admUserLog, Int16 UserId)
        {
