...
namespace AHHA.Infra.Services.Masters
{
    public sealed class OrderTypeService : IOrderTypeService
    {
        ...
        public async Task<M_OrderType> GetOrderTypeByIdAsync(string RegId, Int16 CompanyId, Int32 OrderTypeId, Int32 UserId)
        {
            try
            {
                var result = await _repository.GetQuerySingleOrDefaultAsync<M_OrderType>(RegId, $"SELECT OrderTypeId,OrderTypeCode,OrderTypeName,OrderTypeCategoryId,CompanyId,Remarks,IsActive,CreateById,CreateDate,EditById,EditDate FROM dbo.M_OrderType WHERE OrderTypeId={OrderTypeId} AND CompanyId IN (SELECT distinct CompanyId FROM Fn_Adm_GetShareCompany({CompanyId},{(short)Modules.Master},{(short)Master.OrderType}))");
        ...
        public async Task<SqlResponce> AddOrderTypeAsync(string RegId, Int16 CompanyId, M_OrderType OrderType, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.M_OrderType WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.OrderType})) AND OrderTypeCode='{OrderType.OrderTypeCode}' UNION ALL SELECT 2 AS IsExist FROM dbo.M_OrderType WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.OrderType})) AND OrderTypeName='{OrderType.OrderTypeName}'");
        ...
        public async Task<SqlResponce> UpdateOrderTypeAsync(string RegId, Int16 CompanyId, M_OrderType OrderType, Int32 UserId)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        var StrExist = await _repository.GetQueryAsync<SqlResponceIds>(RegId, $"SELECT 2 AS IsExist FROM dbo.M_OrderType WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)Modules.Master},{(short)Master.OrderType})) AND OrderTypeName='{OrderType.OrderTypeName} AND OrderTypeId <>{OrderType.OrderTypeId}'");
