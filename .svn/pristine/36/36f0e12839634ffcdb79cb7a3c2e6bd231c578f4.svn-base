...
namespace AHHA.API.Controllers.Accounts.AR
{
    ...
    : base(memoryCache, mapper, baseServices)
        ...
        [Authorize]
        public async Task<ActionResult<ARInvoiceViewModel>> GetARInvoiceById(Int64 InvoiceId, [FromHeader] HeaderViewModel headerViewModel)
        {
            try
            {
                if (ValidateHeaders(headerViewModel.RegId, headerViewModel.CompanyId, headerViewModel.UserId))
                {
                    ...
                    if (userGroupRight != null)
                    {
                        var ARInvoiceViewModel = await _ARInvoiceService.GetARInvoiceByIdAsync(headerViewModel.RegId, headerViewModel.CompanyId, InvoiceId, string.Empty, headerViewModel.UserId);
