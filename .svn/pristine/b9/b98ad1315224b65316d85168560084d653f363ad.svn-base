...
namespace AHHA.Infra.Services.Masters
{
    public sealed class BankService : IBankService
    {
        ...
        public async Task<SqlResponse> SaveBankAsync(string RegId, Int16 CompanyId, M_Bank m_Bank, Int16 UserId)
        {
            using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                ...
                try
                {
                    ...
                    if (IsEdit)
                    {
                        var DataExist = await _repository.GetQueryAsync<SqlResponseIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.M_Bank WHERE BankId<>0 AND BankId={m_Bank.BankId} ");
                    ...
                    {
                        var codeExist = await _repository.GetQueryAsync<SqlResponseIds>(RegId, $"SELECT 1 AS IsExist FROM dbo.M_Bank WHERE BankId<>0 AND BankCode={m_Bank.BankCode} AND BankName={m_Bank.BankName} ");

                        if (codeExist.Count() > 0 && codeExist.ToList()[0].IsExist == 1)
                            return new SqlResponse { Result = -1, Message = "Bank Code Same" };

                        //Take the Next Id From SQL
                        var sqlMissingResponse = await _repository.GetQuerySingleOrDefaultAsync<SqlResponseIds>(RegId, "SELECT ISNULL((SELECT TOP 1 (BankId + 1) FROM dbo.M_Bank WHERE (BankId + 1) NOT IN (SELECT BankId FROM dbo.M_Bank)),1) AS NextId");
        ...
        public async Task<SqlResponse> AddBankAsync(string RegId, Int16 CompanyId, M_Bank Bank, Int16 UserId)
        {
            using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                try
                {
                    ...
                    if (DataExist.Count() > 0)
                    {
                        ...
                        {
                            return new SqlResponse { Result = -1, Message = "Bank Code Exist" };
                        }
                        else if (DataExist.ToList()[0].IsExist == 2)
                        {
                            return new SqlResponse { Result = -2, Message = "Bank Name Exist" };
                        }
                    }
                    ...
                    if (sqlMissingResponse != null && sqlMissingResponse.NextId > 0)
                    {
                        ...
                        Bank.BankId = Convert.ToInt16(sqlMissingResponse.NextId);
        ...
        public async Task<SqlResponse> UpdateBankAsync(string RegId, Int16 CompanyId, M_Bank Bank, Int16 UserId)
        {
            using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                try
                {
                    if (Bank.BankId > 0)
                    {
                        var DataExist = await _repository.GetQueryAsync<SqlResponseIds>(RegId, $"SELECT 2 AS IsExist FROM dbo.M_Bank WHERE CompanyId IN (SELECT DISTINCT CompanyId FROM dbo.Fn_Adm_GetShareCompany ({CompanyId},{(short)E_Modules.Master},{(short)E_Master.Bank})) AND BankName='{Bank.BankName}' AND BankId <>{Bank.BankId}");
        ...
        public async Task<SqlResponse> DeleteBankAsync(string RegId, Int16 CompanyId, M_Bank Bank, Int16 UserId)
        {
            using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                try
