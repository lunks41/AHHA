...
namespace AHHA.Infra.Services.Masters
{
    public sealed class MasterLookupService : IMasterLookupService
    {
        ...
        public async Task<IEnumerable<CurrencyLookupModel>> GetCurrencyLookupListAsync(string RegId, Int16 CompanyId, Int32 UserId)
        {
            try
            {
                var result = await _repository.GetQueryAsync<CurrencyLookupModel>(RegId, $"SELECT CurrencyId,CurrencyCode,CurrencyName FROM M_Currency WHERE CurrencyId<>0 And IsActive=1 And CompanyId IN (SELECT distinct CompanyId FROM Fn_Adm_GetShareCompany({CompanyId},{(short)Modules.Master},{(short)Master.Currency})) order by CurrencyName");
